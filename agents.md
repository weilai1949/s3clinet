...
- 版本：稳定里程碑后日常发版用 **`v1.0.0-年月日十分秒`**（`v1.0.0-YYYYMMDDHHmmss`）；预发布可用 **`v1.0.0-rcN`**（例如 `v1.0.0-rc1`）。改版本号时运行 `./scripts/release-version.sh`（或手动同步 `CHANGELOG.md`、`docs/API.md`、`Makefile`、`package.json`、`Cargo.toml`、`tauri.conf.json`、`docker-compose.yml`、`README.md`）。
  ...
  - **机器形式**（合法 SemVer，去掉 `v` 前缀）：`web/package.json`、`desktop/package.json`、`desktop/src-tauri/Cargo.toml`、`desktop/src-tauri/tauri.conf.json` 的 `version` 字段（如 `1.0.0-rc1`）。
